...
interface Props {
  ...
  pinRight: string[];
  totalWidth: number;
  portfolioCols?: { id: string; label: string; width: number }[];
}
...
export function SummaryFooter({ bonds, visibleColIds, colWidths, pinLeft, pinRight, totalWidth, portfolioCols = [] }: Props) {
  ...
  return (
    <table style={{ width: Math.max(totalWidth, 800), tableLayout: 'fixed', minWidth: '100%', borderCollapse: 'separate', borderSpacing: 0 }}>
      <tbody>
        <tr className="bg-slate-100 dark:bg-slate-800 font-semibold">
          {portfolioCols.map(col => (
            <td key={col.id}
              style={{ width: col.width, minWidth: col.width, maxWidth: col.width }}
              className="px-2 py-1.5 text-xs border-r border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800"
            />
          ))}
