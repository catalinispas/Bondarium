...
export function AdvancedFilterModal() {
  const { open, setOpen } = useTableStore(useShallow(s => ({
    open: s.advancedFilterOpen,
    setOpen: s.setAdvancedFilterOpen,
  })));
  ...
  return (
    ...
    >
      <div
        ...
        className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 w-full max-w-2xl max-h-[80vh] overflow-y-auto"
      >
        ...
        <div className="p-4">
          <FilterBuilder onClose={() => setOpen(false)} embedded />
        </div>
      </div>
    </div>
  );
